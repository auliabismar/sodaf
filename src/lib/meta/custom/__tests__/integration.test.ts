...
describe('Custom Field Integration', () => {
	...
	describe('DocTypeMeta Integration', () => {
		it('should integrate with DocTypeMeta for field access', async () => {
			...
			// Get DocTypeMeta
			const docTypeMeta = await docTypeEngine.getDocTypeMeta('User');
			expect(docTypeMeta).not.toBeNull();
			
			// Check standard fields
			expect(docTypeMeta?.get_field('name')).not.toBeNull();
			expect(docTypeMeta?.get_field('email')).not.toBeNull();
			
			// Check that custom fields are not in standard DocTypeMeta
			expect(docTypeMeta?.get_field('cf_phone')).toBeNull();
			expect(docTypeMeta?.get_field('cf_age')).toBeNull();
			...
			expect(mergedDocTypeMeta.get_field('name')).not.toBeNull();
			expect(mergedDocTypeMeta.get_field('email')).not.toBeNull();
			expect(mergedDocTypeMeta.get_field('cf_phone')).not.toBeNull();
			expect(mergedDocTypeMeta.get_field('cf_age')).not.toBeNull();
			
			// Check field count
			expect(mergedDocTypeMeta.get_all_fields()).toHaveLength(4);
		});
