...
describe('MigrationHistoryManager', () => {
	...
	describe('initializeHistoryTable', () => {
		it('should create migration history table if not exists', async () => {
			...
			expect(mockDatabase.run).toHaveBeenCalledWith(
				expect.stringMatching(/CREATE TABLE IF NOT EXISTS tabMigrationHistory/)
			);

			// Should create indexes
			expect(mockDatabase.run).toHaveBeenCalledWith(
				expect.stringMatching(/CREATE INDEX IF NOT EXISTS idx_migration_doctype/)
			);
			expect(mockDatabase.run).toHaveBeenCalledWith(
				expect.stringMatching(/CREATE INDEX IF NOT EXISTS idx_migration_status/)
			);
			expect(mockDatabase.run).toHaveBeenCalledWith(
				expect.stringMatching(/CREATE INDEX IF NOT EXISTS idx_migration_timestamp/)
			);
		});
	...
	describe('recordMigration', () => {
		it('should record migration in history table', async () => {
			...
			expect(mockDatabase.run).toHaveBeenCalledWith(
				expect.stringMatching(/INSERT INTO tabMigrationHistory/),
	...
	describe('updateMigrationStatus', () => {
		it('should update migration status in history table', async () => {
			...
			expect(mockDatabase.run).toHaveBeenCalledWith(
				expect.stringMatching(/UPDATE tabMigrationHistory\s+SET status = \?, error = \?\s+WHERE id = \?/),
	...
	describe('getMigrationHistory', () => {
		it('should return migration history for specific DocType', async () => {
			...
			expect(mockDatabase.sql).toHaveBeenCalledWith(
				expect.stringMatching(/SELECT \* FROM tabMigrationHistory\s+WHERE doctype = \?\s+ORDER BY timestamp DESC/),
		...
		it('should return all migration history when no DocType specified', async () => {
			...
			expect(mockDatabase.sql).toHaveBeenCalledWith(
				expect.stringMatching(/SELECT \* FROM tabMigrationHistory\s+ORDER BY timestamp DESC/),
				[]
			);
		...
		it('should limit results when limit specified', async () => {
			...
			expect(mockDatabase.sql).toHaveBeenCalledWith(
				expect.stringMatching(/SELECT \* FROM tabMigrationHistory\s+ORDER BY timestamp DESC\s+LIMIT \?/),
				[10]
			);
		});
	});
	...
	describe('getMigrationById', () => {
		it('should return specific migration by ID', async () => {
			...
			expect(mockDatabase.sql).toHaveBeenCalledWith(
				expect.stringMatching(/SELECT \* FROM tabMigrationHistory\s+WHERE id = \?\s+ORDER BY timestamp DESC\s+LIMIT 1/),
				[migrationId]
			);
	...
	describe('getLatestMigration', () => {
		it('should return latest applied migration for DocType', async () => {
			...
			expect(mockDatabase.sql).toHaveBeenCalledWith(
				expect.stringMatching(/SELECT \* FROM tabMigrationHistory\s+WHERE doctype = \? AND status = \?\s+ORDER BY timestamp DESC\s+LIMIT 1/),
	...
	describe('isMigrationApplied', () => {
		it('should return true when migration is applied', async () => {
			...
			expect(mockDatabase.sql).toHaveBeenCalledWith(
				expect.stringMatching(/SELECT COUNT\(\*\) as count FROM tabMigrationHistory\s+WHERE id = \? AND status = \?/),
	...
	describe('clearHistory', () => {
		it('should clear history for specific DocType', async () => {
			...
			expect(mockDatabase.run).toHaveBeenCalledWith(
				expect.stringMatching(/DELETE FROM tabMigrationHistory WHERE doctype = \?/),
				[testConstants.TEST_DOCTYPE]
			);
		});
		...
		it('should clear all history when no DocType specified', async () => {
			...
			expect(mockDatabase.run).toHaveBeenCalledWith(
				expect.stringMatching(/DELETE FROM tabMigrationHistory/),
				[]
			);
		});
	});
	...
	describe('getMigrationStats', () => {
		it('should return migration statistics for specific DocType', async () => {
			...
			mockDatabase.sql = vi.fn().mockImplementation((query) => {
				// Return valid response for both queries
			...
			expect(mockDatabase.sql).toHaveBeenCalledWith(
				expect.stringMatching(/SELECT\s+COUNT\(\*\)\s+as\s+total,\s+SUM\(CASE\s+WHEN\s+status\s+=\s+\?\s+THEN\s+1\s+ELSE\s+0\s+END\)\s+as\s+applied/),
			...
			expect(stats.pending).toBe(0); // 10 - 8 - 1 - 1 = 0
		...
		it('should return overall statistics when no DocType specified', async () => {
			...
			expect(stats.pending).toBe(0); // 20 - 16 - 2 - 2 = 0
