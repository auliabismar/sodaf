...
describe('MigrationValidator', () => {
	...
	describe('checkDataLossRisks', () => {
		it('should identify column removal risks', async () => {
				...
				removedColumns: [
					{
						...
						column: { name: 'legacy_field', type: 'text' },
						destructive: true
					}
				],
				modifiedColumns: [],
			...
			const risks = await validator.checkDataLossRisks({
				...diff,
				removedColumns: diff.removedColumns.map(col => ({
					...col,
					column: {
			...
			expect(risks[0].mitigation).toContain('Export column data');
		});
		...
		it('should identify type conversion risks', async () => {
			...
			expect(risks[0].mitigation).toContain('Test conversion');
		});
		
		it('should identify table rebuild risks', async () => {
			const diff = complexSchemaDiff;
			...
			expect(tableRebuildRisks[0].mitigation).toContain('maintenance window');
		});
	});
});