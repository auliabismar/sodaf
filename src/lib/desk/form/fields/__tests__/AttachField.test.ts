...
describe('AttachField', () => {
	...
	it('emits error event when validation fails', async () => {
		...
		component = render(AttachField, {
			props: { field, value: [], maxSize: 100, onerror }
		});
		...
		await waitFor(() => {
			...
			// Checking exact argument might be tricky depending on implementation details, 
			// but we can check if it was called.
			// If the component passes a string or array, we can check that.
		});
	});
