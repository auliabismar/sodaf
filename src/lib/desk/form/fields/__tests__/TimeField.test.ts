...
describe('TimeField', () => {
	...
	it('emits change event on value change', async () => {
		...
		component = render(TimeField, {
			props: { field, value: '10:15', onchange }
		});
	...
	it('correctly formats time for storage', async () => {
		...
		component = render(TimeField, {
			props: { field, value: null, timeFormat: '12', onchange }
		});

		const timePicker = screen.getByRole('textbox', { name: /time/i });
		await fireEvent.input(timePicker, { target: { value: '11:45' } });

		expect(onchange).toHaveBeenCalledWith('23:45'); // 11:45 PM should convert to 23:45 in 24-hour format
	});
	...
	it('emits focus and blur events', async () => {
		...
		component = render(TimeField, {
			props: { field, value: '10:15', onfocus, onblur }
		});
