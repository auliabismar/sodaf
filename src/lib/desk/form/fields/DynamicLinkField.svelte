<script lang="ts">
	...
	// Determine optionsField and targetDoctype
	let computedOptionsField = $derived.by(() => {
		if (field.options) {
	...
	let targetDoctype = $derived.by(() => {
		if (!field.options || !computedOptionsField || !formData) return '';

		const lines = field.options.split('\n').filter((line) => line.trim() !== '');
		if (lines.length <= 1) return '';

		const optionsValue = formData[computedOptionsField];
		if (!optionsValue) return '';

		// Find the DocType that matches the options value
		for (let i = 1; i < lines.length; i++) {
			const line = lines[i].trim();
			if (line.toLowerCase() === optionsValue.toLowerCase()) {
				return line;
			}
		}
	...
	// Watch for changes in target DocType and fetch new options
	$effect(() => {
		if (targetDoctype && targetDoctype !== currentDoctype) {
			// Clear current value and options when DocType changes
			value = '';
			searchValue = '';
			options = [];
			fetchOptions();
	...
	async function fetchOptions(searchTerm: string = '') {
		...
		// Skip if we're already loading for the same DocType
		if (isLoading && currentDoctype === targetDoctype) return;
		...
		try {
			...
			console.error('Error fetching dynamic link options:', error);
			options = [];
		} finally {
			isLoading = false;
		}
	...
	onMount(() => {
		// Fetch initial options if value is set and we have a target DocType
		if (value && targetDoctype) {
			fetchOptions(value);
		}
	});
</script>
...
>
	<div class="link-field-container">
		<ComboBox
			...
			placeholder={targetDoctype ? getPlaceholderText() : `Select ${computedOptionsField} first`}
			disabled={isDisabled || !targetDoctype}
			invalid={isInvalid}
